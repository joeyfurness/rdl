...
func TestUnrestrictMultipleLinks(t *testing.T) {
	responses := map[string]UnrestrictedLink{
		"https://hoster.com/file/1": {ID: "1", Filename: "file1.mkv", Filesize: 100, Download: "https://dl.example.com/1"},
		"https://hoster.com/file/2": {ID: "2", Filename: "file2.mkv", Filesize: 200, Download: "https://dl.example.com/2"},
		"https://hoster.com/file/3": {ID: "3", Filename: "file3.mkv", Filesize: 300, Download: "https://dl.example.com/3"},
	}
	...
	links := []string{
		...
		"https://hoster.com/file/2",
		"https://hoster.com/file/3",
	}

	results, errs := UnrestrictLinks(client, links)
	if len(errs) != 0 {
	...
	for i, result := range results {
		expected := responses[links[i]]
		if result.Filename != expected.Filename {
			t.Errorf("result[%d].Filename = %q, want %q", i, result.Filename, expected.Filename)
		}
	}
}
